...
from field_classification import shot_coordinate
...
vid_path = "testing_videos/heuristic_testing_data/two_point/two_point_video_3_29_c_1.mp4"
device = "cuda" if torch.cuda.is_available() else "cpu"

#YOLOV8 Detector
model = YOLO("detection_weights/latest_weights.pt")
roi = ROI(model)
...
ball_hoop_threshold = 0.40   #this is the hyper parameter that can be handled further
ball_hand_threshold = 3     #determines how close the ball is to the hand bounding box
...
while(cap.isOpened()):
  ...
  #################################getting all of the player_ids and their respective keypoints ####################
    ...
    for cord, cls in zip(coordinates, classes):
      ...
      x2 = int(cord[2])
      y2 = int(cord[3])
      visframe = cv2.rectangle(detect_frame, (x1, y1), (x2, y2), (255, 0, 0), 2)
