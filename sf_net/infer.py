...
from sf_net.semseg.utils.utils import timer
from sf_net.semseg.utils.visualize import draw_text
...
if __name__ == '__main__':
    ...
    with console.status("[bright_green]Processing..."):
        if test_file.is_file():
            ...
            segmap = sf_net.semseg.predict(str(test_file), cfg['TEST']['OVERLAY'])
            segmap.save(save_dir / f"{str(test_file.stem)}.png")
        else:
            files = test_file.glob('*.*')
            for file in files:
                console.rule(f'[green]{file}')
                segmap = sf_net.semseg.predict(str(file), cfg['TEST']['OVERLAY'])
